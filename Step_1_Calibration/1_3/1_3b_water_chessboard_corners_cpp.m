...
% chessboard in water imaged concurrently by the left and right cameras of a
...
waterImageList='waterchesslist'; % the name of the text file containing the directory list of the reference chessboard images
...
watLeftImgName='cam1_water';
watRightImgName='cam2_water';
...
chessFileID = fopen(sprintf('%s/%s.txt',calDirectory, waterImageList'),'w'); % name of the text file

fprintf(chessFileID,'%s\n',sprintf('%s/%s/%s.png',calDirectory,imagesFolder,watLeftImgName));
fprintf(chessFileID,'%s\n',sprintf('%s/%s/%s.png',calDirectory,imagesFolder,watRightImgName));
fclose(chessFileID);

name=sprintf('"%s/%s.txt"',calDirectory, waterImageList);
...
movefile(sprintf('%s/cal/chess_left_rw.xml',calDirectory),sprintf('%s/cal/chess_left_water.xml',calDirectory))
movefile(sprintf('%s/cal/chess_right_rw.xml',calDirectory),sprintf('%s/cal/chess_right_water.xml',calDirectory))
...
[ waterChessLeft, waterChessRight ] = get_chessboard_points( calDirectory,'water' );
...
save(waterChessPixelsName,'waterChessLeft', 'waterChessRight');